<template>
      ...
      <el-table :data="tableData" border :class="tableData.length ? '' : 'noborder'"
        ...
        <el-table-column :min-width="item.minWidth" v-for="(item, index) in columnBase" :key="index" :width="item.width" :label="item.label"
          ...
          <template #default="scope">
            ...
            {{ option.find(f => f.value == scope.row[item.prop]) ? option.find(f => f.value == scope.row[item.prop]).label : '--' }}
          </span> 
  ...
  <detailDialog v-if="dialogType.showDialog" :blocktrade="true" :orderNo="orderNo" @close="closeDialogType" />
</template>

<script>
export default { name: 'blocktradeSearch' };
</script>
<script setup>
import { apiQueryList } from '/@/api/modules/blocktrade'
...
Bus.on('update:blocktradeSearch', () => {
  getDataList()
})

const tabPosition = ref('blocktradeSearch')
const tabChange = ()=>{
  router.replace({
...
if(sessionStorage['blocktradeSearchTimeTanges']){
  timeRanges.value = JSON.parse(sessionStorage['blocktradeSearchTimeTanges'])
}
const optionStatus = [
  {
    value: 'all',
...
const searchForm = reactive({
  params: sessionStorage['blocktradeSearchParams'] || '',
  role: sessionStorage['blocktradeSearchRole'] || 'all',
  status: sessionStorage['blocktradeSearchStatus'] || 'all'
})
...
const columnBase = ref([
  ...
  { prop: 'name', label: '大宗商品',minWidth:gw(110), align: 'center' },
...
const getDataList = (page) => {
  ...
  sessionStorage['blocktradeSearchParams'] = searchForm.params
  sessionStorage['blocktradeSearchTimeTanges'] = JSON.stringify(timeRanges.value)
  ...
  if(sessionStorage['blocktradeSearch']){
    const blocktradeSearchCache = JSON.parse(sessionStorage['blocktradeSearch'])
    if(blocktradeSearchCache.cacheKey == cacheKey){
      tableData.value = blocktradeSearchCache.data
    }else{
  ...
  apiQueryList(send)
    .then(res => {
      ...
      sessionStorage['blocktradeSearch'] = JSON.stringify({
        cacheKey,
        data:res
      })
      tableData.value = res
...
const changeSearch = (s) => {
  searchForm.role = s;
  sessionStorage['blocktradeSearchRole'] = s;
  getDataList();
}
