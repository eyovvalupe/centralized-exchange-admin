...
import blocktradeCtr from './components/blocktradeCtr.vue'
...
const tabPosition = ref('blocktradePos')
const tabChange = ()=>{
  router.replace({
...
const searchValue = ref(sessionStorage['blocktradePosSearchValue'] || 'all')
const searchStr = ref(sessionStorage['blocktradePosSearchStr'] || '')
const searchStrbtn = ref(searchStr.value)

const tableData = computed(() => {
  let list = socketStore.blocktradeOrderList || []
  if (searchValue.value !== 'all') {
    list = socketStore.blocktradeOrderList.filter(f => f.role == searchValue.value)
  }
  if (searchStr.value) {
    list = socketStore.blocktradeOrderList.filter(f => {
...
const getDataList = () => {
  ...
  setTimeout(()=>{
    ...
    sessionStorage['blocktradePosSearchStr'] = searchStr.value
...
const changeSearch = (s) => {
  searchValue.value = s;
  sessionStorage['blocktradePosSearchValue'] = s

}
watch(() => socketStore.sokcetWS, (ws) => {
  if (ws) {
    socketStore.send('blocktradeorder', 'sub')
  }
}, { immediate: true })
</script>
