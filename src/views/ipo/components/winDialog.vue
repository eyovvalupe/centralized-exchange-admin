<template>
   <el-dialog :close-on-click-modal="false" width="420" class="reset-el-styte" v-model="show" title="中签管理" :append-to-body="true">
    <div class="from-info">

      <!-- <p><label>UID</label><span>{{  props.data.uid }}</span></p> -->
      <p><label>认购数量</label><span><b class="h1" style="line-height: 12px;">{{ props.data.volume }}</b></span></p>
      <p><label>杠杆</label><span>
          <span class="tag status-yellow" v-if="props.data.lever || props.data.lever > 0">x{{ props.data.lever }}</span>
          <span v-else>无</span>
        </span></p>
    ...
    <el-form ref="ruleForm" label-position="top" :model="form" :rules="rules">
      <el-form-item label="中签数量" prop="winning" label-width="80" required>
        <el-input v-model="form.winning" @input="changeNumber" style="width: 100%;"></el-input>
      </el-form-item>
    </el-form>
