...
import { getAllUsers, apiRemoveBlacklist, apiBlacklist, apiMsgRead, apiSetRemark, apiCreateChatInfo } from '/@/api/modules/service/index.api'
...
const getAllList = (params) => {
 ...
 const Reg = new RegExp(params, 'i');
  const filterObj = {};
  for (let key in useService.userList) {
    const item = useService.userList[key];
    const name_match = item.username?.indexOf(params) >= 0;
    const msg_match = item.lastmsg?.indexOf(params) >= 0;
    if (msg_match || name_match) {
      const tmp_obj_con = { ...item };
      if (msg_match) {
        tmp_obj_con.lastmsg = tmp_obj_con.lastmsg?.replace(Reg, `<b class='keyword'>${params}</b>`);
      }
      if (name_match) {
        tmp_obj_con.username = tmp_obj_con.username?.replace(Reg, `<b class='keyword'>${params}</b>`);
      }
      filterObj[key] = tmp_obj_con
    }
  }
  dataList.value = filterObj;
}
const clearUserList = () => {
  msgUserList.value = [];
  state.userName = ''
